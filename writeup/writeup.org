...
On two of the models we build (mixture density and convolution based) the results are
...
the weighted averaging ensembles without going very /deep/ (which might need
more instances to fit).
